        ...
        if( jQuery('#migla_ipn_choice').is(':checked') ){ ipn = 'front';  }
		  ...
		  jQuery.ajax({
			...
			success: function(msg) {                         
                  ...
                  var isChatBack = 'no';
                  if( jQuery('#migla_ipn_chatback').is(':checked') ){ isChatBack = 'yes'; }
		  jQuery.ajax({
			type : "post",
			url :  miglaAdminAjax.ajaxurl, 
        ...
        if( jQuery('#miglaPaypalSendFEC').is(':checked') ){ fec = 'yes';  }
		  ...
		  jQuery.ajax({
			...
			data : { action : "miglaA_update_me", key:'migla_payment', 
   			         value  : jQuery('input[name=miglaPaypal]:checked').val() },
			success: function(msg) { 
...
jQuery('#miglaCSSButtonPickerSave').click(function(){
 ...
 jQuery('#miglaSaveCCInfo').click(function(){
  ...
  jQuery('#miglaUpdatePaypalPDTSettings').click(function(){
  
         var isPDT = 'no';
        if( jQuery('#migla_using_pdt').is(':checked') ){ isPDT = 'yes';  }

		jQuery.ajax({
			type : "post",
			url :  miglaAdminAjax.ajaxurl, 
			data : {action: "miglaA_update_me", key:'migla_using_pdt', value: isPDT },
			success: function(msg) {			  
			}
		})  ; //ajax	
		...
		jQuery.ajax({
         ...
         if( jQuery('#migla_pdt_using_ca').is(':checked') ){ isCA = 'yes';  }
		  jQuery.ajax({
				...
				success: function(msg) {  
                      saved('#miglaUpdatePaypalPDTSettings');				
				}
		  })  ; //ajax		  
  }); 
