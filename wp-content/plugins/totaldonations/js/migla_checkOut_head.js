	   ...
	   if( paypal_method == 'paypal_standard' )
	   {
		...
		successUrl = successUrl + sessionid;
			...
			jQuery.ajax({
			  ...
			  success: function(msg2) {
					...
					hiddenForm .append("<input type='hidden' name='notify_url' value='"+ miglanotifyurl +"' >");
				 sendtoPaypal(); 
			  }
			  //,async:false
			}); //ajax 
		...
		if ( isRepeat == 'no' ) {
			   ...
			   jQuery.ajax({
				   ...
				   url :  miglaajaxurl,  
				   data :  { 
					  action    : "miglaA_paypalDirectPaymentExecutor" , 
					  donorinfo : mdata, 
					  session   : sessionid,
					  ccnumber  : cc_number,
					  cctype    : cc_type ,
				   ...
				   success: function( promsg1 ) {
					 if( promsg1 == '1' ){
						  ...
						  alert(promsg1);
						  jQuery('#miglapaypalcheckout_pro').slideDown();
			   ...
			   jQuery.ajax({
				   type : "post",
				   url :  miglaajaxurl,  
				   data :  { action    : "miglaA_paypalSubscriptionExecutor" , 
					  donorinfo : mdata, 
					  session   : sessionid,
					  ccnumber  : cc_number,
					  cctype    : cc_type ,
      ...
      if( isRepeat == 'no')
      {
            ...
            jQuery.ajax({
                     ...
                     success: function(msg_auth) {
                       if( msg_auth == '1' ){
                           var url = miglasuccessurl + "?" + "thanks=thanks";
							  url = url + '&firstname=' + postdata['firstname'];
							  url = url + '&lastname=' + postdata['lastname'] ;
							  url = url + '&amount=' + postdata['amount'] ;						   
                           window.location.replace(url);
