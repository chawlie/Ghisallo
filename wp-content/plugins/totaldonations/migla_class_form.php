...
class migla_form_creator
{
   ...
   public function __construct( $fid )
   {
	  ...
	  /*
	  if( !(self::IS_AFORM_EXIST) )
	  {
			self::IS_AFORM_EXIST 	= true;
			self::AFORM_EXIST 		= 1;
	  }else{
		self::AFORM_EXIST = self::AFORM_EXIST + 1;		  
	  }
	  */
   }
   ...
   public function get_form()
   {
	  ...
	  { 
		...
		$out .= __("You can not have more than one Total Donations form in a single page or post", "migla-donation");  
...
function mg_makeInputCheckTag( $id, $label, $type, $code, $filter, $req , $uid)
{
  ...
  $out .= "</label>"; 
   ...
   $out .= "<label style='display:none' class='idfield' id='".$code.$id."'></label>";
   if( strcmp($req, 'required') == 0 ){ 
   $out .= "<div class='checkbox'><label for='".$uid."'><input checked type='checkbox' id='".$id."' class='check-control ".$code." ".$req."' value='".
           __("yes", "migla-donation")."'/></label></div>";
   }else{
...
if( $this->TYPE_AMOUNT_BTN == 'button' )
{
   ...
   {    
	   ...
	   $out .= "<div class='col-sm-5 col-xs-12'>";
		...
		$out .= "<div class='radio-inline miglaCustomAmount'><label for='miglaCustomAmount".$idx."' style='background-color:".$bglevelcolor.";". $borderCSS."' class='migla_amount_lbl'>";
   ...
   {   
	   ...
	   $out .= "<input type='radio' value='custom' id='miglaAmount".$idx."' name='miglaAmount' class='migla_amount_choice migla_custom_amount'><div>". $custom_amount_text."</div>";
