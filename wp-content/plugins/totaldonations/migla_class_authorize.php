		 ...
		 if( isset($_POST['x_response_code']) && ( $_POST['x_response_code'] == 1  || $_POST['x_response_code'] == '1' ) )
		 {
		   if( isset($_POST['x_subscription_id']) ) //check if it is a sub
		   {
			   ...
			   if( $_POST['x_subscription_paynum'] == '1' )
			   {
				...
				add_post_meta( $new_id, 'miglad_paymentdata', $_POST );
			   }else{
				   //Ongoing recurring
				  // add_option( ( 'migla_auth_POST_ongoing_'. date('Y-m-d-h-i:s') . "_" . $old_id ), $_POST );
			 ...
			 add_option( ('migla_auth_POST_one_'.date('Y-m-d-h-i:s')), $_POST );
